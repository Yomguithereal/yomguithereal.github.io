...
Let's say we need to parse a very large, hundreds of gigabytes large, csv file containing urls we will need to fetch. To ensure we are not going to run out of memory while parsing the whole file, we need to stream it instead, reading it line by line.


```js
csvLines.forEachStreamedLine(line => {
  fetch(line.url);
});
```
...
At this point, the astute reader has easily noticed that we just defeated the purpose of streaming the csv file since we now need to commit a hefty amount of urls to memory.

And here lies the issue: we need a way not to fetch the same urls too much while also making sure we won't run out of memory.
