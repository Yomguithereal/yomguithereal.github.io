...
<p>
  ...
  But why is this order relevant? Wouldn't it be better to record the number of times each item was accessed so we can evict the least frequently used item instead?
</p>
...
Usually, to implement a proper LRU cache, you need the two following ingredients:

1. A [hashmap](https://en.wikipedia.org/wiki/Hash_table)-like data structure able to retrieve values associated to arbitrary keys – such as strings – efficiently. In JavaScript you can either use the ES6 `Map` or any plain object `{}`: remember that our cache is no different from a fixed-capacity key-value store.
2. A way to store our items in the order of their last access. What's more, you will need to be to move items around efficiently. That's why people naturally lean toward a [doubly-linked list](https://en.wikipedia.org/wiki/Doubly_linked_list) for the job.
...
And here is how you could use such a cache:
...
// If you get `2`, it won't be the LRU key anymore
cache.get(2);
>>> 'two'
...
<div className="paragraph">
  ...
  To be able to implement a LRU cache, you will need to implement a doubly-linked list to make sure we are able to store our items in order of their last access: the most recently used item starting the list and the least recently used one ending it.
...
Note that each time, you will need to update the head pointer and that, sometimes, you will also need to update the tail pointer.

Finally, if the cache is already full and a yet unknown key needs to be inserted, you will need to pop the last item from the list to make place to prepend the new one.
...
// You can read/set items very naturally
array[10] = 34;
array[10];
>>> 34
